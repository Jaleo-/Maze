...
public class Cells implements Serializable {
	...
	static final int CW_BOT_BOUND = 64; // 2^6
	...
	public int width;  //changed from private to public...  should change back!!!!!!!!//
	public int height ;
	public int[][] cells;
	/**
	 * Constructor
	...
	public void initialize() {
		...
		for (x = 0; x != width; x++) {
			for (y = 0; y != height; y++) {
				//sets all cells to virgin to show that they have not been visited.
	 ...
	 */
	//Added a check to make sure there were no indexing errors when the border was used to see if an area overlapped.
	//The check used to have the loop start and end chosen so that there was at least one cell between area and any existing room.  This did not make
	//sense so I made it go away.
	 ...
	 * Only used in markAreaAsRoom.  Can't be used for the outer border.
	 * @param x coordinate of cell
	 * @param y coordinate of cell
	 * @param dx direction x, in { -1, 0, 1} obtained from dirsx[]
	 * @param dy direction y, in { -1, 0, 1} obtained from dirsy[]
	 ...
	 * Can't create a room that touches the outer border!
	 * @param rw room width
	...
	public boolean isVirgin(int x, int y) {  //Changed to public for testing!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
	...
	public boolean hasBoundOnRight(int x, int y){
		//return (cells[x][y] & MazeBuilder.CW_RIGHT_BOUND) != 0;
		return hasMaskedBitsTrue(x, y, CW_RIGHT_BOUND);
	}
	public boolean hasBoundOnLeft(int x, int y){
		//return (cells[x][y] & MazeBuilder.CW_LEFT_BOUND) != 0;
		return hasMaskedBitsTrue(x, y, CW_LEFT_BOUND);
	}
	public boolean hasBoundOnTop(int x, int y){
		//return (cells[x][y] & MazeBuilder.CW_TOP_BOUND) != 0;
		return hasMaskedBitsTrue(x, y, CW_TOP_BOUND);
	}
	public boolean hasBoundOnBottom(int x, int y){
		//return (cells[x][y] & MazeBuilder.CW_BOT_BOUND) != 0;
		return hasMaskedBitsTrue(x, y, CW_BOT_BOUND);
	}
	...
	public boolean hasNoWallOnBottom(int x, int y) {
		//return (cells[x][y] & MazeBuilder.CW_BOT) == 0;
		return hasMaskedBitsFalse(x, y, CW_BOT);
	}
	public boolean hasNoWallOnTop(int x, int y) {
		//return (cells[x][y] & MazeBuilder.CW_TOP) == 0;
		return hasMaskedBitsFalse(x, y, CW_TOP);
	}
	public boolean hasNoWallOnLeft(int x, int y) {
		//return (cells[x][y] & MazeBuilder.CW_LEFT) == 0;
		return hasMaskedBitsFalse(x, y, CW_LEFT);
	}
	public boolean hasNoWallOnRight(int x, int y) {
		//return (cells[x][y] & MazeBuilder.CW_RIGHT) == 0;
		return hasMaskedBitsFalse(x, y, CW_RIGHT);
	}
	public boolean hasNoBoundOnRight(int x, int y){
		//return (cells[x][y] & MazeBuilder.CW_RIGHT_BOUND) == 0;
		return hasMaskedBitsFalse(x, y, CW_RIGHT_BOUND);
	}
	public boolean hasNoBoundOnLeft(int x, int y){
		//return (cells[x][y] & MazeBuilder.CW_LEFT_BOUND) == 0;
		return hasMaskedBitsFalse(x, y, CW_LEFT_BOUND);
	}
	public boolean hasNoBoundOnTop(int x, int y){
		//return (cells[x][y] & MazeBuilder.CW_TOP_BOUND) == 0;
		return hasMaskedBitsFalse(x, y, CW_TOP_BOUND);
	}
	public boolean hasNoBoundOnBottom(int x, int y){
		//return (cells[x][y] & MazeBuilder.CW_BOT_BOUND) == 0;
		return hasMaskedBitsFalse(x, y, CW_BOT_BOUND);
	}
	...
	//Checks to see if there is a wall...  basically......  Not sure why this exists........................................
