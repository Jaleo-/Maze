...
public class MapDrawer {
	 ...
	 */
	public void draw_map(GraphicsWrapper gw2, int px, int py, int walk_step, int view_dx, int view_dy, boolean showMaze, boolean showSolution) {
		gw2.setColor("white");
		...
		for (int y = ymin; y <= ymax; y++)
			for (int x = xmin; x <= xmax; x++) {
				...
				gw2.setColor(seencells.hasWallOnTop(x, y) ? "white"  :"gray");
				if ((seencells.hasWallOnTop(x, y) || showMaze) && w)
					gw2.drawLine(nx1, ny1, nx2, ny1);
				...
				gw2.setColor(seencells.hasWallOnLeft(x, y) ? "white" : "gray");
				if ((seencells.hasWallOnLeft(x, y) || showMaze) && w)
					gw2.drawLine(nx1, ny1, nx1, ny2);
			}
		if (showSolution) {
			//draw_solution(gc, offx, offy);
			draw_solution(gw2, offx, offy, px, py) ;
		}
	 ...
	 * @param gw2
	 */
	public void draw_currentlocation(GraphicsWrapper gw2, int view_dx, int view_dy) {
		gw2.setColor("red");
		...
		gw2.fillOval(ctrx-cirsiz/2, ctry-cirsiz/2, cirsiz, cirsiz);
		...
		gw2.drawLine(ctrx, ctry, aptx, apty);
		gw2.drawLine(aptx, apty, aptx2 + ptofx, apty2 + ptofy);
		gw2.drawLine(aptx, apty, aptx2 - ptofx, apty2 - ptofy);
	}
	 ...
	 * @param gw2 to draw lines on
	 * @param offx
	 * @param offy
	 */
	public void draw_solution(GraphicsWrapper gw2, int offx, int offy, int px, int py) {
		...
		if((px>=0 && px<mazew) && (py>=0 && py<mazeh)){
			...
			while (d > 1) {
				...
				// do the graphics
				gw2.drawLine(nx1, ny1, nx1+ndx, ny1+ndy);
