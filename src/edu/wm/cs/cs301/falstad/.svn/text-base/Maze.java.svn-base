...
public class Maze implements Serializable { 
	...
	private void writeObject(ObjectOutputStream out) throws IOException { 
		out.writeInt(mazew);
		out.writeInt(mazeh);
		out.writeInt(roomcount);
		out.writeInt(expectedPartct);
		//if(bsp == null)
			//Log.v(" Null",":)");
		out.writeObject(bsp);
		...
		Log.v("before serialized", ""+view_dx);
		Log.v("before serialized", ""+view_dy);
	...
	private void readObject(ObjectInputStream in) throws IOException, ClassNotFoundException{
		...
		expectedPartct = in.readInt();
		bsp = (BSPNode) in.readObject();
		...
		walk_step =(int) in.readInt();
		view_dx =(int) in.readInt();
		view_dy =(int) in.readInt();
		Log.v("unserialized", ""+view_dx);
		Log.v("unserialized", ""+view_dy);
		ang =(int) in.readInt();
		map_mode = in.readBoolean();
...
//
	public ManualDriver mo = new ManualDriver();
	public Robot walle = new BasicRobot();
//	private Graphics gc;
//
...
//	
	...
	public transient final int view_width = 410;
	public transient final int view_height = 410;
	int zscale = view_height/2;

	public transient final static int map_unit = 128;
	transient final int view_offset = map_unit/8;
	public transient static final int step_size = map_unit/4;
...
//	private Image buffer_img;
	...
	public int py;
	int walk_step;
	public int view_dx; // current view direction
	public int view_dy;
	...
	public int[][] mazedists;
	public Cells seencells ;
	public BSPNode bsp;
...
//	// Drawer to get the first person perspective
	public FirstPersonDrawer firstpersondrawer ;
	...
	public int method; // 0 : default method, Falstad's original code
	...
	public void Graphics(Context context) {
...
//	 * @param gw.gc graphics handler to manipulate screen
//	 */
	private void redrawPlay(GraphicsWrapper gw) {
//		/**
...
//		
		...
		switch(method){
		case 2: 
			mazebuilder = new MazeBuilderKruskal(); //generate with Kruskal's algorithm
			Log.v("Build", "Kruskal");
			break;
		case 1 : 
			mazebuilder = new MazeBuilderPrim();
			Log.v("Build", "Prim");// generate with Prim's algorithm
			break ;
		...
		mazew = skill_x[skill];
		mazeh = skill_y[skill];
		roomcount = skill_rooms[skill];
		expectedPartct = skill_partct[skill];
		mazebuilder.build(this, mazew, mazeh, roomcount, expectedPartct);
