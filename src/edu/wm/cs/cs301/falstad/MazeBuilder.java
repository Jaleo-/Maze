...
public class MazeBuilder implements Runnable {
	...
	protected void generatePathways() {
		int[][] origdirs = new int[width][height] ; //Creates a new 2D array that is the same width and height as cells.
		//x is a chosen to be a random number between 0 and width-1.
		int x = randNo(0, width-1) ;
		int firstx = x ; //Starting position for x
		int y = 0;       // 
		int dir = 0; 	 //The first direction checked is the right wall. 
		int origdir = dir; // 
		cells.setVirginToZero(x, y);
		while (true) { 
			int dx = dirsx[dir], dy = dirsy[dir];
			//Determines which direction to check based on the arrays dirsx and dirsy.  
			// If you cannot go in the given direction, change direction and try again. 
			if (!cells.canGo(x, y, dx, dy)) { 
				dir = (dir+1) & 3; //Adds one to the current direction and mods by three to make sure it falls within the index of the array
				if (origdir == dir) { //If you've tried all the directions and are back to the one you started with... 
					// If you are back to the starting position...
					if (x == firstx && y == 0)
						break; //You've finished generating the pathways.  Return to the generate method. 
					//If you've tried all the directions and you aren't back at the starting point, backtrack.  
					//Find the direction you had moved before 
					int odr = origdirs[x][y];
					dx = dirsx[odr];
					dy = dirsy[odr];
					//Go back to the cell you were at before. 
					x -= dx;
					y -= dy;
					// 
				...
				//If, from the starting location, you can move in the given direction, delete the wall in that direction. 
				cells.deleteWall(x, y, dx, dy);
				//Change the values of x and y to move to the cell that you just deleted the wall into. 
				x += dx;
				y += dy;
				//Mark that the cell you have just entered is no longer a virgin.
				cells.setVirginToZero(x, y);
				//Set the direction in origdirs to the one you just used to create the path.  Pick a new random number to be the next direction you try.
				origdirs[x][y] = dir;
				origdir = dir = randNo(0, 3);
			}
		}
	 ...
	 */
	protected Vector<Seg> generateSegments() {
		int x, y;
	 ...
	 * @param sl
	 */
	protected void setPartitionBitForCertainSegments(Vector<Seg> sl) {
