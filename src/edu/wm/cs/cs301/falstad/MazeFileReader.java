...
public class MazeFileReader {
	...
	private static void compareSegments(Vector<Seg> slist, Vector<Seg> slist2) {
		...
		{
			s = slist.elementAt(i) ;
//			gw = new Color(s.col[0], s.col[1], s.col[2]);
			scolor.setColor(android.graphics.Color.rgb(s.col[0], s.col[1], s.col[2]));
			s2 = slist2.elementAt(i) ;
			s2color.setColor(android.graphics.Color.rgb(s2.col[0], s2.col[1], s2.col[2]));
			if(s.x != s2.x) 
